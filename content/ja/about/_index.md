...
_InterDead（インターデッド）は、スペキュレイティブ・フィクション（SpecFi）の枠組みにおける心理志向ホラー・プロジェクトであり、「死者のためのメッセンジャー」という概念を中心に、ゲームとそれを取り巻くメタユニバースの開発に焦点を当てています。本作のゲームは現在、Zhovten Games スタジオによって開発中です。また、ゲームと並行して、テーブルトークRPGセッションを通じたユニバースの拡張も計画されています。_
...
InterDead には、感情的に負担が大きい、あるいは好まれない可能性のあるテーマが含まれます：
...
- 自殺；
- 人工知能；
- 強制と自律性の喪失（本人の意思に反する行為）；
- ホラー；
- スペキュレイティブ・フィクション（SpecFi）；
- 残虐な殺害描写；
- 宗教的モチーフ。

この種の内容が不快であったり、心理的に強い負担となり得る場合は、プロジェクトの閲覧を続けないことを推奨します。

本コンテンツは成人向けです（18歳以上、またはお住まいの法域における成人年齢以上）。
...
空に向けて撃たれた合図などなかった。私はただ腰を下ろし、書き始めただけだった。私はプロトタイプと、その周囲にすべてのドラマトゥルギーが構築される概念的な枠組みを描き出した。それは、長年にわたって死者との接触を試みてきた科学者によって作られたアプリケーションだった。ある時点で彼は成功するが、それは特定の死者との親密な会話という形ではなかった。

この人物の広い背景、アプリケーションの内部ロジック、そしてその架空の系譜は、プロジェクトのウィキに記録されている。そこでは、これらの要素が一貫した概念モデルの一部として提示されている。このモデルを形作るにあたり、私たちはさまざまな文化的・文学的参照点に依拠した。とりわけ、クライヴ・バーカーの作品（『Books of Blood』『Pulse』ほか）において、「向こう」と「こちら」の境界が、粗雑に縫い合わされたファスナーのように振る舞う点に影響を受けている。これらの作品はプロジェクトの雰囲気やイメージを理解する上で重要だが、物語や構造の論理的な源泉ではない。

現在、本プロジェクトは二つの公開ウィキを運営している。

- [https://interdead.fandom.com/ja/wiki/InterDead_Wiki](https://interdead.fandom.com/ja/wiki/InterDead_Wiki)
- [https://github.com/Zhovten-Games/InterDeadReferenceLibrary](https://github.com/Zhovten-Games/InterDeadReferenceLibrary)

そこには、アプリケーションの内部ロジック、使用される用語、そして全体の概念構造が記録されている。
...
現在、私たちの状況は客観的に見て以前ほど安定していない。個人的なリソースを用いて実施する予定だったいくつかの加速的な施策を断念せざるを得なかった。そのため、当初の計画と比べて、プロジェクトはより慎重かつ段階的に、進行速度や優先順位を見直しながら進められている。
